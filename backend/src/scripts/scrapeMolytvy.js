...
/**
 * Скрапер розділу "Молитви / Гімни" з сайту pryvatri.de
 *
 ...
 *
 * Джерело:   https://pryvatri.de/molytvy
 * Категорія: hymns (ГІМНИ / МОЛИТВИ)
 *
 * Запуск:
 *   node src/scripts/scrapeMolytvy.js            # додати нові, пропустити існуючі
 *   node src/scripts/scrapeMolytvy.js --update    # також оновити існуючі
 */
...
const SECTION = 'molytvy';
const CATEGORY = 'hymns';

/**
 * Перетворює структуру, яку повертає DetailedSongScraper, у формат схеми Song.
 * (Логіка узгоджена з importFromJson.js)
 */
...
async function main() {
  ...
  try {
    console.log('🙏 Скрапінг розділу "Молитви / Гімни" (molytvy → hymns)');
    ...
    for (let i = 0; i < songLinks.length; i++) {
      ...
      try {
        ...
        const doc = {
          ...
          category: CATEGORY,
          tags: [SECTION, 'hymns', 'pryvatri.de', 'structured'],
          isPublic: true,
