...
/**
 * Скрапер розділу "Козацькі пісні" з сайту pryvatri.de
 *
 ...
 *
 * Джерело:   https://pryvatri.de/kosacki
 * Категорія: cossack (КОЗАЦЬКІ ПІСНІ)
 *
 * Запуск:
 *   node src/scripts/scrapeCossack.js            # додати нові, пропустити існуючі
 *   node src/scripts/scrapeCossack.js --update    # також оновити існуючі
 */
...
const SECTION = 'kosacki';
const CATEGORY = 'cossack';
...
async function main() {
  ...
  try {
    console.log('⚔️  Скрапінг розділу "Козацькі пісні" (kosacki → cossack)');
    ...
    for (let i = 0; i < songLinks.length; i++) {
      ...
      try {
        ...
        const doc = {
          ...
          category: CATEGORY,
          tags: [SECTION, 'cossack', 'pryvatri.de', 'structured'],
          isPublic: true,
