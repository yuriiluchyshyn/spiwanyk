...
/**
 * Скрапер розділу "Колядки / Щедрівки" з сайту pryvatri.de
 *
 ...
 *
 * Джерело:   https://pryvatri.de/koladky
 * Категорія: carols (КОЛЯДКИ / ЩЕДРІВКИ)
 *
 * Запуск:
 *   node src/scripts/scrapeCarols.js            # додати нові, пропустити існуючі
 *   node src/scripts/scrapeCarols.js --update    # також оновити існуючі
 */
...
const SECTION = 'koladky';
const CATEGORY = 'carols';
...
async function main() {
  ...
  try {
    console.log('⭐ Скрапінг розділу "Колядки / Щедрівки" (koladky → carols)');
    ...
    for (let i = 0; i < songLinks.length; i++) {
      ...
      try {
        ...
        const doc = {
          ...
          category: CATEGORY,
          tags: [SECTION, 'carols', 'pryvatri.de', 'structured'],
          isPublic: true,
